...
def build_query(bbox: str) -> str:
    angkot_pattern = "angkot|angkutan kota|mikrolet|kwk|koasi"
    return f"""
    [out:json][timeout:180];
    (
      relation["route"="share_taxi"]({bbox});
      relation["route"="minibus"]({bbox});
      relation["route"="bus"]["name"~"{angkot_pattern}",i]({bbox});
      relation["route"="bus"]["operator"~"{angkot_pattern}",i]({bbox});
      relation["route"="bus"]["network"~"{angkot_pattern}",i]({bbox});
    );
    out geom;
    """
...
async def fetch_angkot_relations() -> list[dict[str, Any]]:
    """Fetch every region and deduplicate relations crossing bounding boxes."""
    elements_by_id: dict[tuple[str, int], dict[str, Any]] = {}
    headers = {"User-Agent": "transit-engine/1.0 (+https://github.com/)"}

    async with httpx.AsyncClient(timeout=180.0) as client:
        for region_index, (region_name, bbox) in enumerate(REGIONS.items()):
            ...
            if elements and region_index < len(REGIONS) - 1:
