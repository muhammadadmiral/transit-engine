...
def test_relation_geometry_reverses_member_to_keep_route_continuous() -> None:
    coordinates = _relation_coordinates(relation(1))
...
def test_parser_rejects_unrelated_generic_bus_but_accepts_named_angkot_bus() -> None:
    unrelated = relation(1, route="bus", name="TransJakarta BRT 1")
    angkot = relation(2, route="bus", name="Angkot K02")

    dataset = parse_osm_relations([unrelated, angkot])

    assert dataset.segments
    assert all("TransJakarta" not in segment.service_name for segment in dataset.segments)
    assert {segment.route_id.split(":")[2] for segment in dataset.segments} == {"2"}
